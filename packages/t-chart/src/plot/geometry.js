import { compose, indexOf } from "@tinyvis/t-utils";
...
// type 为几何图形的 type
export function initialize({ data, type, encodings: E, transforms: transformsOptions, statistics: statisticsOptions = [], styles }) {
  ...
  Object.entries(encodings).forEach(([encodingName, encodingValue]) => {
    if (encodingValue.type === ENCODING_TYPES.CONSTANTS) {
      constantStyles[encodingName] = encodingValue.value;
    }
  ...
  const { index: I, values: transformedValues } = statistics({ index, values: encodingValues });
